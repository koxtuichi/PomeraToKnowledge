...
14. **月次収入**: 日記で収入を記録した記述。「knowbeから〇〇万円の振込」「今月の収入は〇〇円」のような記述から抽出する。
    ...
    - amount: 収入額（整数・円）
    - note: 補足（税抜き/税込み等）
15. **勉強記録**: 勉強したことの記録。「〇〇を勉強した」「〇〇について学んだ」のような記述から抽出する。
    - subject: 科目・分野名（例: TypeScript, 機械学習, 簿記）
    - learning_type: "概念理解" / "実践演習" / "資料読解" / "問題演習" / "その他"
    - mastery: 習熟度 0.0〜1.0（0.0=全く知らない, 1.0=完全に理解）
    - key_concepts: 学んだ主要概念リスト（文字列配列）
    - questions: この勉強中に生まれた疑問リスト（文字列配列）
16. **問題（学習）**: 勉強中に生まれた未解決の疑問や問題。
    - status: "未解決" / "解決済"
    - difficulty: "易" / "中" / "難"
...
| 発見する | 勉強記録 → 問題（学習） | 勉強中に発見した疑問・問題 |
| 解説する | 勉強記録 → 概念 | 勉強記録がある概念を解説する |
| 向上させる | 勉強記録 → 目標 | 勉強記録が目標の達成に貢献する |
...
- "study"   : 勉強・学習に関係するノード（勉強記録、問題（学習）ノードは必ずこのcontext）
...
# ノード構造
{
  ...
  "subject": "勉強記録ノードの場合のみ: 科目・分野名",
  "learning_type": "勉強記録ノードの場合のみ: 概念理解/実践演習/資料読解/問題演習/その他",
  "mastery": "勉強記録ノードの場合のみ: 習熟度 0.0〜1.0",
  "key_concepts": "勉強記録ノードの場合のみ: 学んだ概念リスト（文字列配列）",
  "questions": "勉強記録ノードの場合のみ: 生まれた疑問リスト（文字列配列）",
  "difficulty": "問題（学習）ノードの場合のみ: 易/中/難"
}
