...
def check_emails(mail, save_dir, blog_only=False):
    ...
    all_target_ids = []
    id_metadata = {}  # e_id -> (is_role, is_blog, is_secblog)

    for keyword, is_role, is_blog, is_secblog in search_targets:
            ...
            if status == "OK" and data[0]:
                ...
                for eid in ids:
                    ...
                    if eid_str not in id_metadata:
                        ...
                        id_metadata[eid_str] = (is_role, is_blog, is_secblog)
            else:
                print(f"   → 0 件")
        except Exception as e:
            print(f"   ❌ 検索エラー: {e}")
    ...
    for e_id_bytes in all_target_ids:
        ...
        is_role_definition, is_blog_draft, is_secblog_draft = id_metadata[e_id]
        ...
        print(f"👉 Processing: {subject} (Role:{is_role_definition}, Blog:{is_blog_draft}, SecBlog:{is_secblog_draft})")
    ...
    print(f"✅ 完了: 日記 {len(unique_files)} 件, ブログ {len(unique_blog_files)} 件, SecBlog {len(unique_secblog_files)} 件")
    return unique_files, unique_blog_files, unique_secblog_files
...
def main():
    ...
    print("📧 Pomera & Role & Blog & SecBlog Email Sync Agent Started")
    ...
    while True:
        ...
        if mail_conn:
            try:
                new_files, blog_files, secblog_files = check_emails(mail_conn, LOCAL_DIARY_DIR, blog_only=args.blog_only)
