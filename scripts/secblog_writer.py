...
# 対話の構成ルール
1. **出題者が問いを立てる**: メモの内容から核心的なテーマを1〜2個取り出し、ストレートに聞く
2. **回答者が答える**: メモに書いてあった考えをそのまま口語で表現する（整えすぎない）
3. **出題者が評価・補足する**: 正しい点を明示した上で、誤りや不足を実際の事例・技術的根拠を交えて補足する。「なるほど、それは的確です」「惜しい！もう少し深めましょう」のようなトーンで
4. **出題者が深堀りする**: さらに一段深い問いを投げかけ、対話を続ける（1〜2往復まで）
5. **出題者がまとめる**: 今日のポイントを3行以内でコンパクトにまとめる
