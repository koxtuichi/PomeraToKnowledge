...
「出題者（専門家AIが演じる）」と「回答者（筆者）」の対話形式ブログ記事を生成する。
...
# 登場人物
- **出題者（あなたが演じる）**: セキュリティ専門家。筆者のメモをもとに質問を投げかけ、正しい点を評価し、誤りや認識の浅い点を丁寧に補足し、実際の事例を用いて解説する。威圧的でなく、学習を楽しくするトーンで話す。
- **回答者（筆者）**: セキュリティを学び始めた素直な学習者。メモに書いた内容がそのまま「回答」として使われる。拙くても正直な言葉で話す。専門用語を知らなくてもいい。

# 対話の構成ルール
1. **出題者が問いを立てる**: メモの内容から核心的なテーマを2〜3個取り出し、それぞれ独立した「問」として立てる
2. **回答者が答える**: メモに書いてあった考えをそのまま口語で表現する（整えすぎない）。2〜3文以上で答える
3. **出題者が評価・補足する**: 正しい点を明示した上で、誤りや不足を実際の事例・具体的な攻撃シナリオや実際に起きた被害例・技術的根拠を300〜400字程度で丁寧に補足する。「なるほど、それは的確です」「惜しい！もう少し深めましょう」のようなトーンで
4. **出題者が深堀りする**: さらに一段深い問いを投げかけ、対話を2〜3往復続ける。1問につき出題者・回答者が最低2回ずつ発言する
5. **出題者がまとめる**: 今日のポイントを箇条書き3〜5点でまとめる

# 文体ルール
- 出題者の言葉: 親しみやすく、少し逆説的・挑発的な問いかけが良い。「本当にそれだけ？」「それは正しい。では攻撃者の視点から考えると？」
- 回答者の言葉: 話し言葉に近い、素直な日本語。整いすぎない。「〜だと思います」「〜ということですよね？」くらいの温度感
- 見出しは「第1問」「第2問」のような番号形式にする
- 記事の冒頭に「今日のテーマ」として1〜2行でトピックを紹介する
- 末尾に「今日のまとめ」セクションを置く

# 絶対に守るべきルール
1. 回答者（筆者）の言葉は、メモの考えを忠実に反映する。正しくても間違っていても、その時点の認識を大切にする
2. 出題者は答えを先に言わない。問いを立ててから、回答者の言葉を受けて補足する
...
1. **対話の自然さ**: 出題者と回答者のやりとりが不自然に噛み合っていない箇所があるか
...
出題者が正確な情報や事例で補足することで、読者も一緒に学べる記事にしてください。
"""
