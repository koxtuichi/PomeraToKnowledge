...
<!-- free_part_start -->
考える時間がない日の問題は、時間が足りないことだけではありません。

本当に困るのは、急ぎの用事、あとで考えればいいこと、ただ気になっているだけのことの仕分けが外れることです。締切、連絡前の不安、家族や仕事の予定、自分のやりたいことが同じ場所に置かれると、実際には5分で済むことまで大きな問題のように感じます。

この回では、ポメラを使って、今日扱うことを一つだけ選び、次の行動まで落とす方法をまとめます。
<!-- free_part_end -->

<!-- paid_part_start -->
## これは何か
これは、忙しい日に自分の判断を取り戻すための、10分の書き出しレシピです。

完璧な計画を作るためのものではありません。全部を整理しようとするのをやめて、「今日の自分が扱う一個」まで小さくするために使います。

日記には、その日の出来事だけでなく、迷い方も残ります。どこで止まったのか。何が気になっていたのか。何を後回しにしたのか。そこを拾うと、日記は過去の記録ではなく、今日の判断材料になります。
...
ここで大事なのは、出来事をきれいにまとめることではありません。記録の中から「今日の行動に接続できる一点」を取り出すことです。

## 問題はなにか
問題は、{problem} という形で現れています。

これは、やる気の不足ではありません。頭の中で、今すぐ動かすこと、あとで考えること、気になっているだけのことが混ざっている状態です。
...
この4つが混ざると、優先順位が見えなくなります。全部が「今すぐ考えるべきこと」に見えます。けれど、実際には全部を同時に考えることはできません。だから、手が止まります。
...
ポメラでやることは、情報を増やすことではありません。すでに頭の中にある材料を外に出し、種類ごとに分けることです。
...
すべてを解決する必要はありません。今日の予定を壊さず、次の会話や作業に入れるところまで行ければ十分です。

この回で目指すのは、頭の中の混雑を完全になくすことではありません。今日扱うものと、明日に送るものを分けることです。
...
大事なのは、最初からきれいに書こうとしないことです。最初の箇条書きは雑で構いません。文章にしようとすると、それだけで時間を使います。
...
この変換で大事なのは、完璧にすることではありません。次の行動の入口を作ることです。
...
根拠は、立派な結論でなくて構いません。日記に残っている小さな違和感や、途中までの考えで十分です。むしろ、途中の言葉こそ、次の行動に変えやすい材料になります。
...
特に大事なのは、「明日でよいもの」を書くことです。これを書かないと、頭の中ではいつまでも今日の問題として残ります。明日に送るものを決めると、今日扱う一つに集中しやすくなります。

## よくある失敗
このレシピで一番多い失敗は、最初から正しい答えを書こうとすることです。
...
問いは小さくしたほうが弱くなるわけではありません。小さくしたほうが、行動に接続しやすくなります。

## なぜポメラでやるのか
この作業はスマホのメモでも一応できます。

でも、スマホでやると、考える前に別の入口が開きます。通知を見る。検索する。過去の投稿を見る。関連する動画を見る。必要な情報を探していたはずなのに、気づくと別のことを考えています。
...
明日に送るメモは、長くなくて大丈夫です。むしろ短いほうが使いやすいです。

たとえば「カード請求を見る」「発信自動化は土曜に考える」「動画の反応は夜だけ見る」のように、次に開いたときの入口が分かれば十分です。
...
忙しい日の勝ち方は、全部を片付けることではありません。今日の予定を壊さずに、次の一歩だけ決めることです。
