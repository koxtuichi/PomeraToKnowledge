"""
main.py — Cloud Function エントリポイント：ブログ記事生成

GASからHTTPでPOSTされた BLOGメールの本文を受け取り、
ブログ記事を生成してはてなブログに下書き投稿する。

処理フロー:
  1. リクエストからブログ草案を取得
  2. Cloud Storageからナレッジグラフ・文体ガイドを読込
  3. Geminiでブログ記事を生成
  4. 自己レビューと修正
  5. はてなブログに下書き投稿
...
import gcs_io
import blog_article_writer
import hatena_publisher


@functions_framework.http
def process_blog(request):
    """GASからHTTPで呼ばれるブログ処理エントリポイント。"""
    ...
    try:
        ...
        print(f"📝 ブログ草案を受信: 件名={subject}, 文字数={len(body)}")
        ...
        context = ""
        if graph_data:
            context = blog_article_writer.extract_blog_context(graph_data)

        # ── ブログ記事を生成 ──
        print("🔄 Geminiでブログ記事を生成中...")
        article_data = blog_article_writer.generate_blog_article(
            body, context, style_guide=writing_style
        )
        ...
        gcs_io.save_text_to_gcs(f"blog_ready/{date_str}_{safe_title}.md", md_content)
        ...
        gcs_io.save_json_to_gcs(f"blog_ready/{date_str}_{safe_title}_meta.json", meta)

        # ── GitHubにも保存 ──
        gcs_io.push_file_to_github(
            f"blog_ready/{date_str}_{safe_title}.md",
            md_content,
            f"Auto-sync: Blog article {title} [skip ci]"
        )
